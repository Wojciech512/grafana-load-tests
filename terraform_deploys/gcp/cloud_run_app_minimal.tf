resource "google_cloud_run_v2_service" "django_public_minimal" {
  name                = "praca-magisterska-django-app-minimal"
  ...
  template {
    service_account = google_service_account.proxy["minimal"].email
    ...
    containers {
      ...
      env {
        ...
        value = "/cloudsql/${google_sql_database_instance.postgres_minimal.connection_name}"
      }
    }
    ...
    volumes {
      ...
      cloud_sql_instance {
        instances = [
          google_sql_database_instance.postgres_minimal.connection_name,
        ]
      }
    }
...
resource "google_cloud_run_service_iam_member" "django_invoker_minimal" {
  location = var.project_region
  project  = google_project.this.project_id
  service  = google_cloud_run_v2_service.django_public_minimal.name
