...
resource "azurerm_private_endpoint" "postgres_pe" {
  for_each = {
    db_b1ms = azurerm_postgresql_flexible_server.db_b1ms.id
    db_b2s  = azurerm_postgresql_flexible_server.db_b2s.id
    db_b2ms = azurerm_postgresql_flexible_server.db_b2ms.id
  }
