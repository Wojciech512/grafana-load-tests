...
module "virtual_network" {
  ...
  virtual_network_name_input = "webapp-postgress-virtual-network"
  ...
  subnets_input = {
    app-subnet = {
      ...
      delegation_name_input  = "app-delegation"
      service_name_input     = "Microsoft.Web/serverFarms"
      service_actions_input  = ["Microsoft.Network/virtualNetworks/subnets/action"]
    }
    database-subnet = {
    ...
    private-endpoint-subnet = {
      address_prefixes_input = ["10.0.3.0/24"]
...
module "private_dns" {
  ...
  resource_group_name_input = module.resource_group.name_output
  virtual_network_id_input  = module.virtual_network.vnet_id
}
...
module "private_endpoint" {
  ...
  resource_group_name_input                    = module.resource_group.name_output
  subnet_id_input                              = module.virtual_network.subnet_ids["private-endpoint-subnet"]
  subresource_names_input                      = ["postgresqlServer"]

  private_dns_zone_group_name_input = "dns-group"
  private_dns_zone_ids_input        = [module.private_dns.id]
}
...
module "database" {
  ...
  version_input                       = "16"
  public_network_access_enabled_input = true
...
module "firewall" {
  ...
  firewall_rule_config_input = {
    database-firewall-db-b1ms = {
      name_input             = "allow_all_b1ms"
      server_id_input        = module.database.database_ids["database-b1ms-postgres"]
      start_ip_address_input = "0.0.0.0"
      end_ip_address_input   = "255.255.255.255"
    }

    database-firewall-db-b2s = {
      name_input             = "allow_all_b2s"
      server_id_input        = module.database.database_ids["database-b2s-postgres"]
      start_ip_address_input = "0.0.0.0"
      end_ip_address_input   = "255.255.255.255"
    }

    database-firewall-db-b2ms = {
      name_input             = "allow_all_b2ms"
      server_id_input        = module.database.database_ids["database-b2ms-postgres"]
      start_ip_address_input = "0.0.0.0"
      end_ip_address_input   = "255.255.255.255"
    }
  }
