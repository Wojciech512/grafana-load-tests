...
export default function userJourney() {
  const page = http.get(`${BASE_URL}/account/login`);
  const csrf = page.cookies["csrftoken"][0].value;
  const headers = {
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
      "X-CSRFToken": csrf, // header akceptowany przez Django
      Cookie: `csrftoken=${csrf}`, // utrzymuje sesję
    },
  };
  ...
  res = http.post(
    `${BASE_URL}/cart/`,
    JSON.stringify({ product_id: 68226, quantity: 1 }),
    regHeaders,
  );
